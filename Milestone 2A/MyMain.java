...
public class MyMain {
    ...
    public static void main(String[] args) throws FileNotFoundException, InterruptedException {
        ...
        // Parsing YAML File
        InputStream inputstream = new FileInputStream(new File("./src/main/resources/Milestone2A.yaml"));
        Yaml yaml = new Yaml();
    ...
    // Process Task
    private static void processTask(Map<?, ?> allTaskMap, String taskname, String parent) throws InterruptedException, FileNotFoundException {
        ...
        String FuncType = (String) alltasks.get("Function");
        if(FuncType.equals("TimeFunction"))
        {
        ...
        {
            Map<?, ?> mytask = (Map<?, ?>) alltasks.get("Inputs");
            String filename = (String) mytask.get("Filename");
            String str = parent + "." + taskname + " Executing DataLoad"  + "(" + filename + ")";
            DataLoad(str, filename);
        }

        logger.info(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSSSSS")) + ";" + parent + "." + taskname +  " Exit");
    }

    private static void DataLoad(String str, String filename) throws FileNotFoundException {

        logger.info(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSSSSS")) + ";" + str);
...
//        }
    ...
    // Process Flow
    private static void processFlow(Map<?, ?> allTaskMap, String taskname, String parent) throws InterruptedException, FileNotFoundException {
        logger.info(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSSSSS")) + ";" + parent + "." + taskname + " Entry");
